	...
	public void dropDatabase() {
		...
		try {
			String delete = "drop SEQUENCE if exists sequenza_id;" + "drop table if exists messaggio;"
					+ "drop table if exists presenza;" + "drop table if exists lezione;" + "drop table if exists aula;"
					+ "drop table if exists iscritto;" + "drop table if exists studente;"
					+ "drop table if exists docente;" + "drop table if exists contiene;"
	...
	public void createDatabase() {
		...
		try {
			...
			String create = "create SEQUENCE sequenza_id;"
					...
					+ "create table utente(matricola VARCHAR(255) primary key,nome VARCHAR(255),cognome varchar(255),email VARCHAR(255),password VARCHAR(255),data_nascita DATE,codice_fiscale VARCHAR(16),corsodilaurea VARCHAR(255) REFERENCES corsodilaurea(\"codice\"),ruolo int,verifycode VARCHAR(255),imagepath VARCHAR(255),descrizione text);"
					+ "create table corso(\"codice\" bigint primary key, nome VARCHAR(255),anno int,descrizione text,requisiti text,giorni VARCHAR(255),ore_lezioni int,ore_esercitazioni int,materiale text,\"docente\" VARCHAR(255) REFERENCES utente(\"matricola\"),corsodilaurea VARCHAR(255) REFERENCES corsodilaurea(\"codice\"),cfu int,cognomeDocente VARCHAR(255), nomeDocente VARCHAR(255),data_inizio Date, data_fine Date);"
					...
					+ "create table messaggio(\"id\" bigint primary key, data timestamp, testo text,matricola_mitt VARCHAR(255) REFERENCES utente(\"matricola\"),matricola_dest VARCHAR(255) REFERENCES utente(\"matricola\"),letta boolean);"
